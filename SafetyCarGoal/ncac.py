...
def train_ac():
    ...
    while n <= n_train:
        ...
        #print(len(log_prob_grad))
        # print("before-------------")
        # for i in list(policy.parameters()):
        #     print(i)

        # for i, tensor in enumerate(log_prob_grad):
        #     print("Shape of tensor {}: {}".format(i,tensor.shape))
        # # Compute the Fisher Information Matrix
        # # This is a placeholder - you'll need to implement this yourself
        # FIM = compute_FIM(policy, state,action)
        #log_prob_g = torch.tensor([log_prob_grad] )
        #FIM = torch.matmul(log_prob_g,torch .t(log_prob_g))
        listt =[]
        ...
        for gradd, param in zip(log_prob_grad, policy.parameters()):
            ...
            # if(mm == 0):
            #     param.data = param.data.view(nn)
            # else:
            #     param.data = param.data.view(nn, mm)
        #print(len(prev_FIM))
        # #print(Categorical(probs))
        # print("after-------------")
        # for i in list(policy.parameters()):
        #     print(i)
        # policy.l1.weight.data = listt[0]
        # policy.l1.bias.data = listt[1]
        # policy.l2.weight.data = listt[2]
        # policy.l2.bias.data = listt[3]
        Y = Y + a*(cost - Y)
...
f3 = open("plotting_nac_const_sdt_final.txt", "a")
n_seed = 10
